...
#'    vehicle body types using `TABWEIGHT` for weights, and generates a
...
#'    Defaults to `"x axis"`.
#' @param y_plot_label A character string for the y-axis label.
#'    Defaults to `"y axis"`.
#'
...
get_btype_graphs <- function(vius1, var, plot_title = "BAR CHART",
  ...
  custom_data <- vius1 |>
    ...
    dplyr::summarize(
      total = round(sum(TABWEIGHT * {{ var }}) / sum(TABWEIGHT), 2),
      .groups = "drop"
    )
